...
import type { RepositoryV2 } from "../core/repository";
...
const createEvent = (repo: RepositoryV2) =>
...
const dispatchFailed = (repo: RepositoryV2, dispatchId: string) =>
  repo.mutate(async (tx) =>
    safeTry(async function* () {
...
const getTargetDispatch = (repo: RepositoryV2, dispatchId: string) =>
...
/** @internal */
export const testRepositoryPersistsLostDispatch = async (
  repo: RepositoryV2,
) => {
  const got = await safeTry(async function* () {
    const { dispatchId } = yield* await createEvent(repo);
...
/** @internal */
export const testRepositoryListOngoingDispatches = async (
  repo: RepositoryV2,
) => {
  // Create dispatches.
...
export const testRepositoryListEventDispatches = async (repo: RepositoryV2) => {
